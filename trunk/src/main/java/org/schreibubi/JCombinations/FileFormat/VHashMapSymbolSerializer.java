 ...
 * 
 */
public class VHashMapSymbolSerializer implements IMarshaller, IUnmarshaller, IAliasable {
	...
	public void marshal(Object obj, IMarshallingContext ictx) throws JiBXException {
		...
		if (!(obj instanceof VHashMap))
			...
			while (iter.hasNext()) {
				...
				if (value != null) {
					ctx.startTagAttributes(this.m_index, VHashMapSymbolSerializer.ENTRY_ELEMENT);
					ctx.attribute(this.m_index, VHashMapSymbolSerializer.KEY_ATTRIBUTE, key);
					ctx.attribute(this.m_index, VHashMapSymbolSerializer.TYPE_ATTRIBUTE, value.getTypeString());
					ctx.closeStartContent();
					ctx.content(value.getValueString());
					ctx.endTag(this.m_index, VHashMapSymbolSerializer.ENTRY_ELEMENT);
				}
			}
	...
	public Object unmarshal(Object obj, IUnmarshallingContext ictx) throws JiBXException {
		...
		while (ctx.isAt(this.m_uri, VHashMapSymbolSerializer.ENTRY_ELEMENT)) {
			String key = ctx.attributeText(null, VHashMapSymbolSerializer.KEY_ATTRIBUTE, null);
			Symbol.SymType type = Symbol.getTypeFromString(ctx.attributeText(null,
					VHashMapSymbolSerializer.TYPE_ATTRIBUTE, null).toUpperCase());
			String text = ctx.parseElementText(this.m_uri, VHashMapSymbolSerializer.ENTRY_ELEMENT);
			Symbol sym = null;
			switch (type) {
			case INTEGER:
