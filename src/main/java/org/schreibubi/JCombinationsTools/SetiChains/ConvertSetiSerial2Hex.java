...
public class ConvertSetiSerial2Hex extends SetiSerial2Converter {
	...
	public String getHexString(SetiChainData setiChainData, int hexLength)
			throws Exception {
	...
	public VArrayList<Integer> getMRSValues(SetiChainData setiChainData,
			int hexLength) throws Exception {

		// if hex length is negative we automatically determine the required
		// length.
		if (hexLength < 0) {
		...
		for (int i = 0; i < hexLength; i++) {
			int r = 0;
			for (int k = 0; k < 32; k++) {
				r = r + (int) Math.pow(2, k)
						* (resulting_data[i * 32 + k] ? 1 : 0);
			}
			iList.add(r);
		}
		return iList;
		...
		for (int i = 0; i < hexLength; i++) {
			for (int j = 0; j < parallelWidth; j++) {
				if (i * parallelWidth + j < setiChainData.getSize()) {
					resulting_data[i * 32 + commandPins.get(j)] = setiChainData
							.getCommandBit(i * parallelWidth + j);
					resulting_data[i * 32 + dataPins.get(j)] = setiChainData
							.getDataBit(i * parallelWidth + j);
					resulting_data[i * 32 + maskPins.get(j)] = !setiChainData
							.getDataMaskBit(i * parallelWidth + j);
				} else {
					// hex is longer than seti chain so we fill it up with
					// default values
